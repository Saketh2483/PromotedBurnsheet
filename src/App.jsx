  ...
  const respond = async (text) => {
    ...
    if (isLeaveRelated) {
      try {
        ...
        const halfMatch = /\b(?:first\s*half|morning|FN|forenoon)\b/i.test(lower) ? "first" : /\b(?:second\s*half|afternoon|afternoo\w*|AN|evening)\b/i.test(lower) ? "second" : null;        // Parse date references (supports ranges like "from 7 may to 10 may", "5 may - 8 may", single dates like "7 may", "tomorrow")
        const dateRangeMatch = text.match(/(?:from\s+)?(\d{1,2}\s*(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\w*(?:\s+\d{2,4})?)\s*(?:to|-)\s*(\d{1,2}\s*(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\w*(?:\s+\d{2,4})?)/i);
        const singleDateMatch = text.match(/\b(\d{1,2}\s*(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\w*(?:\s+\d{2,4})?)\b/i);
        const simpleDateMatch = text.match(/\b(tomorrow|today|next\s+\w+|\d{1,2}[\/\-]\d{1,2}[\/\-]?\d{0,4})\b/i);
        const leaveDate = dateRangeMatch ? `${dateRangeMatch[1]} to ${dateRangeMatch[2]}` : (singleDateMatch ? singleDateMatch[1] : (simpleDateMatch ? simpleDateMatch[1] : null));
        ...
        const res = await fetch(`${API_BASE}/leave/`, {
        ...
        });        const data = await res.json();        if (data.status === "awaiting_confirmation") {
          setLeaveMode(true);
          setLeaveIdentifier(identifier);
          ...
          setLeaveMode(true);
          setLeaveIdentifier(identifier);
          setLeaveContext({ identifier, leaveDays, permHours, leaveDate, half: halfMatch, pendingField: "date" });
          setMsgs(prev => [...prev, { sender: "assistant", text: `🏖️ **Leave Management**\n\n${data.message}` }]);
    ...
    if (leaveMode && leaveContext && leaveContext.pendingField) {
      ...
      if (pendingField === "half") {
        ...
        const dateRangeReply = text.match(/(?:from\s+)?(\d{1,2}\s*(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\w*(?:\s+\d{2,4})?)\s*(?:to|-)\s*(\d{1,2}\s*(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\w*(?:\s+\d{2,4})?)/i);
        const singleDateReply = text.match(/\b(\d{1,2}\s*(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\w*(?:\s+\d{2,4})?)\b/i);
        const simpleDateReply = text.match(/\b(tomorrow|today|next\s+\w+|\d{1,2}[\/\-]\d{1,2}[\/\-]?\d{0,4})\b/i);
        const parsedDate = dateRangeReply ? `${dateRangeReply[1]} to ${dateRangeReply[2]}` : (singleDateReply ? singleDateReply[1] : (simpleDateReply ? simpleDateReply[1] : null));
        if (parsedDate) {
          const ctx = { ...leaveContext, leaveDate: parsedDate, pendingField: null };
